...
public class DukeApplicationTest extends ApplicationTest {
  ...
  protected static void javafxRun(Runnable action) {
  ...
  protected void press(Scene scene, KeyCode key) {
    javafxRun(() -> scene.getOnKeyPressed()
        .handle(new KeyEvent(KeyEvent.KEY_PRESSED, key.getChar(), key.getName(), key,
  ...
  protected void release(Scene scene, KeyCode key) {
    javafxRun(() -> scene.getOnKeyReleased()
        .handle(new KeyEvent(KeyEvent.KEY_RELEASED, key.getChar(), key.getName(), key,
            false, false, false, false)));
  }

  protected void click(Scene scene, int x, int y) {
    javafxRun(() -> scene.getOnMouseClicked()
        .handle(new MouseEvent(MouseEvent.MOUSE_CLICKED, x, y, x, y, MouseButton.PRIMARY, 1,
            false, false, false, false, true, false, false, true, false, false, null)));
  }

  protected void moveTo(Scene scene, int x, int y) {
    javafxRun(() -> scene.getOnMouseMoved()
        .handle(new MouseEvent(MouseEvent.MOUSE_MOVED, x, y, x, y, MouseButton.NONE, 0,
  ...
  protected void clickOn(Shape s) {
    ...
    simulateAction(s, () -> s.getOnMouseClicked()
        .handle(new MouseEvent(MouseEvent.MOUSE_CLICKED, x, y, x, y, MouseButton.PRIMARY, 1,
            false, false, false, false, true, false, false, true, false, false, null)));
  }
